    ...
    private System.Collections.IEnumerator MoveRoutine() {
        while(true) {
            ...
            if(movingUp) {
                ...
                if(movingUp) {
                timesMoved++;
                yield return StartCoroutine(MoveObject(transform.position + Vector3.down * upDistance,moveSpeed));
                yield return StartCoroutine(RotateObject(rotationAngle,rotationSpeed));
                
                movingUp = false;
            } else {
                timesMoved++;
                yield return StartCoroutine(MoveObject(transform.position + Vector3.up  * downDistance,moveSpeed));
                yield return StartCoroutine(RotateObject(rotationAngle,rotationSpeed));
                movingUp = true;
